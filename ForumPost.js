// ForumPost.js
import { supabase } from './vendor/bootstrap/js/supabaseClient.js';
...
const authorName = row => (row.isanonymous ? 'Anonymous' : row.User?.firstname || 'Unknown');
...
async function toggleRepliesForButton(btn) {
  ...
  // Prefer the closest replies container 
...
async function refreshAllThreadsFor(parentId) {
  ...
  for (const c of containers) {
    // refresh  so the new reply appears immediately
